/**
	-- INTERNAL MODULE --
	This module is used exclusively by Gesture.  When a dom element has an event listener registered, 
	it creates an instance of this module to hold all the event handlers for the dom element.  
	Every dom element gets a new GestureBase instance. 
*/
import { Device } from "@ff0000-ad-tech/ad-external";
import { Styles } from "@ff0000-ad-tech/ad-view";
...
class GestureBase {
  ...
  _handleDrag(event) {
    ...
    if (G._isDragging) {
      ...
      if (
        Math.abs(G._sX - mouseX) > G._give ||
        Math.abs(G._sY - mouseY) > G._give
      ) {
        G._isDragging = true;
